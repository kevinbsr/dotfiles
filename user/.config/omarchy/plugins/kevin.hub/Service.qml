...
Item {
  ...
  function selectActivePlayer() {
    ...
    // An explicit pick from the sources list is an answer to the question
    // everything below exists to guess at, so it comes first.
    //
    // Upstream ranked it near-last, behind "whoever holds a PipeWire playback
    // stream" — and Spotify keeps its stream open while paused where a
    // browser drops its own. Clicking a paused browser source therefore set
    // the preference and changed nothing on screen, which is a source list
    // that ignores clicks.
    //
    // Nothing is stranded by this: syncPlayingOrder clears the key when its
    // player disappears, and runAction re-points it at whatever a media key
    // actually landed on, so ordinary use keeps dragging it back to the
    // player making sound.
    if (preferred) return preferred

    var streamCandidate = streamPlayer || streamProxy
    return oldestPlayingPlayer(true) || oldestPlayingPlayer(false) || streamCandidate || trackPlayer || trackProxy || controllablePlayer || controllableProxy || identityPlayer || identityProxy || null
  }
