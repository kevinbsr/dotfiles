...
MENU_CMD="walker --dmenu -p 'Select option'" # Change to rofi/fuzzel/dmenu as needed

get_theme_color() {
  local color_name=$1
  local default_color=$2
  local css_file="$HOME/.config/omarchy/current/theme/waybar.css"
  
  if [ -f "$css_file" ]; then
    local hex
    hex=$(grep -E "@define-color[[:space:]]+${color_name}[[:space:]]+" "$css_file" | head -n1 | awk '{print $3}' | tr -d ';')
...
--status)
  if tailscale_status; then
    T=$(get_theme_color "color2" "green")
    F=$(get_theme_color "color8" "red")
    I="none"
    colors=()
    ...
    *) ip_index="" ;;
    ...
    jq -nc --arg txt "" --arg tip "Tailnet: ""$tailnet"$'\n'"Exit node: ${exitnode:-none}"$'\n\n'"$self"$'\n'"$peers" \
